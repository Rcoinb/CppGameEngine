...
{
	return this->CubeInFrustum(pos.x, pos.y, pos.z, dim.x, dim.y, dim.z);
}
...
bool Frustum::CubeInFrustum(glm::vec3& pos, AABB& aabb)
{
	float width = aabb.GetTransV1()->x - aabb.GetTransV2()->x;
	float height = aabb.GetTransV1()->y - aabb.GetTransV2()->y;
	float depth = aabb.GetTransV1()->z - aabb.GetTransV2()->z;
	return CubeInFrustum(pos.x, pos.y, pos.z, std::abs(width), std::abs(height), std::abs(depth));
}
